/*admin_role_dept管理,作者:alvin198761@163.com,日期:2019-11-17 21:00:17*/
<template>
      <div>
            <Form inline :label-width="70"  style="text-align: left">
                                                        ...
                                                        <InputNumber placeholder="请输入菜单主键" v-model="form.deptId" clearable></InputNumber>
                                                        </FormItem>
                                                                                                    <FormItem label="菜单主键" prop="deptId">
                                                        <InputNumber placeholder="请输入菜单主键" v-model="form.deptId" clearable></InputNumber>
                                                        </FormItem>
        ...
        <RoleDeptDialog ref="dialog" :refresh="refresh"></RoleDeptDialog>
  </div>
</template>
<script>
import RoleDeptDialog from './RoleDeptDialog.vue';
import RoleDeptListExpand from './RoleDeptListExpand.vue';
  export default {
    components: {
     RoleDeptDialog,
     RoleDeptListExpand
    ...
    data: function () {
      ...
      return {
        tableHeader: [
          { type: 'expand',  width: 50, render: (h, params) => { return h(RoleDeptListExpand, {  props: {row: params.row}  })  }  },
                            {title: '主键', key: 'id', slot:'col_id' },
                            {title: '主键', key: 'id', slot:'col_id' },
                            {title: '角色主键', key: 'roleId', slot:'col_roleId' },
                            {title: '角色主键', key: 'roleId', slot:'col_roleId' },
                            {title: '菜单主键', key: 'deptId', slot:'col_deptId' },
                            {title: '菜单主键', key: 'deptId', slot:'col_deptId' },
    ...
    methods: {
      refresh() {
        ...
        that.$postBody("/api/roleDept/queryPage", requestData).then(res => {
              that.loading = false;
              if (res.errcode != 0) {
                that.$Message.error({content: res.errmsg ? res.errmsg : '获取admin_role_dept列表失败'});
                return ;
              }
		          that.dataList = res.replydata.content;
		          that.total = res.replydata.totalElements;
		        }).catch(res => {
		          that.$Message.error({ content: '获取admin_role_dept列表失败：' + res });
		        });
      },
      doAdd() {
      ...
      doDelete(row) {
        ...
        that.$Modal.confirm({
          ...
          closable:true,
          onOk: () => {
            this.$post('/api/roleDept/delete', {id: row.id }).then(res => {
                  if (res.errcode != 0) {
                    that.$Message.error({content: res.errmsg ? res.errmsg : '获取admin_role_dept列表失败'});
                    return ;
                  }
