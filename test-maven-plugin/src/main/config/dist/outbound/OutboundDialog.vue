/*出库记录新增与修改},作者:alvin198761@163.com,日期:2019-11-17 21:00:17*/
<template>
            ...
            <Form :model="form" ref="form" :rules="rules" :label-width="100">
                  <Row>
                        <i-col>
                                                        ...
                                                        <Input placeholder="送货状态" v-model="form.status" clearable/>
                                                        </FormItem>
                                                                                                                <FormItem label="备注" prop="remark">
                                                        <Input placeholder="备注" v-model="form.remark" clearable/>
                                                        </FormItem>
                                                                                                                <FormItem label="date" prop="date">
                                                        <DatePicker type="date" placeholder="date" v-model="form.date" style="width: 100%" clearable></DatePicker>
                                                         </FormItem>
                                                                                    </i-col>
                      </Row>
                </Form>
      ...
      return {
        ...
        rules: {
          ...
          //  {min: 1, max: 10,   message: '备注长度不正确', trigger: 'blur'},
                    ...
                    date : [
                         {required: true, type:'date', message: '请输入date', trigger: 'blur'},
          //  {min: 1, max: 10, type:'date', message: 'date长度不正确', trigger: 'blur'},
                    ],
                }
      }
    },
    methods: {
      save() {//新增及修改记录
        ...
        this.$refs['form'].validate((valid) => {
          ...
          that.$postBody("/api/outbound/" + that.dialogMode, that.form).then(res => {
          that.show = false;
            if (res.errcode != 0) {
