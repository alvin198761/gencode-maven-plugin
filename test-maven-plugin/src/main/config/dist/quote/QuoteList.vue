/*报价管理,作者:alvin198761@163.com,日期:2019-11-17 21:00:17*/
<template>
      <div>
            <Form inline :label-width="70"  style="text-align: left">
                            ...
                            <Option   value="-1"  >-请选择-</Option>
                                                        ...
                                                        <DatePicker type="date" placeholder="请选择报价日期" v-model="form.date" clearable></DatePicker>
                                                        </FormItem>
                                                                                                    <FormItem label="备注" prop="remark">
                                                        <Input placeholder="请输入备注" v-model="form.remark"/>
                                                        </FormItem>
                                                                                                    <FormItem label="单据状态" prop="status">
                                                        <Input placeholder="请输入单据状态" v-model="form.status"/>
                                                        </FormItem>
...
import QuoteListExpand from './QuoteListExpand.vue';
  export default {
    components: {
     QuoteDialog,
     QuoteListExpand
    ...
    data: function () {
      ...
      return {
        tableHeader: [
          { type: 'expand',  width: 50, render: (h, params) => { return h(QuoteListExpand, {  props: {row: params.row}  })  }  },
                            {title: '主键', key: 'id', slot:'col_id' },
                            {title: '客户id', key: 'cusId', slot:'col_cusId' },
                            {title: '报价日期', key: 'date', slot:'col_date' },
                            {title: '备注', key: 'remark', slot:'col_remark' },
                            {title: '单据状态', key: 'status', slot:'col_status' },
    ...
    methods: {
      refresh() {
        ...
        that.$postBody("/api/quote/queryPage", requestData).then(res => {
              that.loading = false;
              if (res.errcode != 0) {
                that.$Message.error({content: res.errmsg ? res.errmsg : '获取报价列表失败'});
                return ;
              }
		          that.dataList = res.replydata.content;
		          that.total = res.replydata.totalElements;
		        }).catch(res => {
		          that.$Message.error({ content: '获取报价列表失败：' + res });
		        });
      },
      doAdd() {
      ...
      doDelete(row) {
        ...
        that.$Modal.confirm({
          ...
          closable:true,
          onOk: () => {
            this.$post('/api/quote/delete', {id: row.id }).then(res => {
                  if (res.errcode != 0) {
                    that.$Message.error({content: res.errmsg ? res.errmsg : '获取报价列表失败'});
                    return ;
                  }
