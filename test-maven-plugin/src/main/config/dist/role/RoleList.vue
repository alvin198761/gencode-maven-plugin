/*admin_role管理,作者:alvin198761@163.com,日期:2019-11-17 21:00:17*/
<template>
      <div>
...
import RoleListExpand from './RoleListExpand.vue';
  export default {
    ...
    data: function () {
      ...
      return {
        tableHeader: [
          { type: 'expand',  width: 50, render: (h, params) => { return h(RoleListExpand, {  props: {row: params.row}  })  }  },
    ...
    methods: {
      refresh() {
        ...
        that.$postBody("/api/role/queryPage", requestData).then(res => {
              that.loading = false;
              if (res.errcode != 0) {
                that.$Message.error({content: res.errmsg ? res.errmsg : '获取admin_role列表失败'});
                return ;
              }
		          that.dataList = res.replydata.content;
		          that.total = res.replydata.totalElements;
		        }).catch(res => {
		          that.$Message.error({ content: '获取admin_role列表失败：' + res });
		        });
      },
      doAdd() {
      ...
      doDelete(row) {
        ...
        that.$Modal.confirm({
          ...
          closable:true,
          onOk: () => {
            this.$post('/api/role/delete', {id: row.id }).then(res => {
                  if (res.errcode != 0) {
                    that.$Message.error({content: res.errmsg ? res.errmsg : '获取admin_role列表失败'});
                    return ;
                  }
