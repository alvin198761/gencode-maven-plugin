/*admin_role_func新增与修改},作者:alvin198761@163.com,日期:2019-11-17 21:00:17*/
<template>
            ...
            <Form :model="form" ref="form" :rules="rules" :label-width="100">
                  <Row>
                        <i-col>
                                                        ...
                                                        <InputNumber placeholder="菜单主键" v-model="form.userId"  style="width: 100%"  clearable></InputNumber>
                                                        </FormItem>
                                                                                                                <FormItem label="菜单主键" prop="userId">
                                                        <InputNumber placeholder="菜单主键" v-model="form.userId"  style="width: 100%"  clearable></InputNumber>
                                                        </FormItem>
    ...
    methods: {
      save() {//新增及修改记录
        ...
        this.$refs['form'].validate((valid) => {
          ...
          that.$postBody("/api/roleFunc/" + that.dialogMode, that.form).then(res => {
          that.show = false;
            if (res.errcode != 0) {
      ...
      addDialog() {//新增
        this.title = "新增admin_role_func";
        this.dialogMode = "save";
        this.form = this.initForm();
        this.show = true;
      },
      editDialog(row) {//修改
        this.title = "修改admin_role_func";
